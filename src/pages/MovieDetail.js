  ...
  useEffect(() => {
    async function fetchMovie() {
      const response = await fetch(
        `https://api.themoviedb.org/3/movie/${params.id}?api_key=250f6bb14526533c704bfb845c4ad385`
      );
    ...
    <main>
      <section className="flex justify-around flex-wrap py-5">
        ...
        <div className="max-w-2xl text-gray-700 text-lg dark:text-white">
          <h1 className="text-4xl text-center lg:text-left font-bold py-4">
            {movie.title}
          </h1>
          ...
          {movie.genres ? (
            <p className="my-7 flex flex-wrap gap-2">
              {movie.genres.map((genre) => (
                <span
                  key={genre.id}
                  className="mr-2 p-2 border rounded border-gray-700 dark:border-white"
                >
                  {genre.name}
                </span>
              ))}
